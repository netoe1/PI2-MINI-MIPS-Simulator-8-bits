...
void escrever_end_mem_dados(CPU *p,uint16_t addr,int8_t valor){
...
void escrever_registrador(CPU *p, uint8_t id,int8_t valor){
...
int8_t ler_registrador(CPU *p, uint8_t id){
    // Se for r0, retorna 0
    if(id == 0) return 0;
